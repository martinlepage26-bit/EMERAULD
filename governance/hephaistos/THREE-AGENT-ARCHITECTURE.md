...
canonical_path: governance/hephaistos/THREE-AGENT-ARCHITECTURE.md
backlink_count: 3
backlinks:
...
**Status:** Implemented as the active repository instruction model  
**Implementation:** Controlling files now exist for all three agents in `AGENTS.md`, `HEPHAISTOS.md`, `QUEEN-KEYPORT.md`, `HERMES.md`, `ORCHESTRATION.md`, and the Anthropic-side `CLAUDE.md` / `.claude/agents/` surfaces  
**Next:** Keep the three-agent files synchronized when authority or routing rules change
...
The three-agent model is not only a design note. It is implemented through the following control files:

- Root dispatcher: `../AGENTS.md`
- Repo-local package dispatcher: `AGENTS.md`
- Agent contracts:
  - `HEPHAISTOS.md`
  - `QUEEN-KEYPORT.md`
  - `HERMES.md`
- Orchestration and handoff logic: `ORCHESTRATION.md`
- Anthropic project entrypoint: `CLAUDE.md`
- Anthropic agent definitions:
  - `../.claude/agents/hephaistos.md`
  - `../.claude/agents/queen-keyport.md`
...
**HEPHAISTOS Consented Frame validation (executed via `diamond-eyes` skill):**
...
**Queen Keyport Consented Frame validation (may consult `diamond-eyes` outputs):**
...
**Hermes Consented Frame validation (may consult `diamond-eyes` outputs):**
