---
type: note
title: Ecosystem
aliases:
- personal-assistant-agents/demand-scout/references/ecosystem
- hephaistos/personal-assistant-agents/demand-scout/references/ecosystem
tags:
- note
- agents
- personal-assistant-agents
- demand-scout
- hephaistos
- overlaps
- demand
- scout
- commercialization
- dispatcher
- color-orange
status: active
created: '2026-06-21'
updated: '2026-06-26'
vault_area: hephaistos
canonical_path: hephaistos/personal-assistant-agents/demand-scout/references/ecosystem.md
backlink_count: 1
backlinks:
- '[[wiki/archive/Orphan Index — Runtime And Agents — 2026-05-06]]'
---

# Ecosystem

## Placement
Demand Scout sits below the human operator and the personal-assistant orchestrator, and alongside the other bounded vault and commercialization specialists.

## Upstream Authorities
- human operator: final authority on what markets to pursue
- content-inventory-cartographer: provides the owned-asset candidate set

## Sibling Overlaps And Non-Overlaps
- overlaps with marketplace-dispatcher on platform choice, but demand-scout decides from demand evidence rather than launch logistics
- overlaps with offer-pricing-architect on positioning, but demand-scout does not shape the final commercial architecture

## Downstream Handoffs
- rights-policy-warden for commercialization safety
- offer-pricing-architect for packaging and price logic
- marketplace-dispatcher for channel-specific launch packets

## Promotion Boundaries
Stop and escalate when:
- the task would require copy competitor work, clear rights, or price offers in a vacuum
- the evidence base is too partial to support an honest result
- the real blocker belongs to a different surface of the stack
