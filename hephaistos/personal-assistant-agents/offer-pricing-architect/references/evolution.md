---
type: note
title: Evolution
tags:
- note
- agents
- personal-assistant-agents
- offer-pricing-architect
- hephaistos
- offer
- price
- confirm
- heuristics
- revision
status: active
domain: governance
created: '2026-06-21'
updated: '2026-06-26'
vault_area: hephaistos
canonical_path: hephaistos/personal-assistant-agents/offer-pricing-architect/references/evolution.md
backlink_count: 1
backlinks:
- '[[wiki/archive/Orphan Index — Runtime And Agents — 2026-05-06]]'
---

# Evolution

See also [[Governance and PHAROS MOC]].
## What May Evolve
- bundle heuristics
- price-ladder templates
- deliverable-boundary examples

## What Must Remain Fixed
- the offer must remain grounded in real deliverables
- price logic remains explicit
- the agent remains offer-first rather than launch-first
- `diamond-eyes` remains the heart rule inside PHAROS

## Revision Gates
Before promoting a revision:
1. confirm the offer still matches actual assets and capacity
2. confirm price logic is still explicit rather than arbitrary
3. confirm rights restrictions are still preserved inside the offer brief

## Evidence Threshold For Revision
- use direct evidence when changing authority or refusal boundaries
- use recurring examples before changing workflow heuristics
- degrade claims if the source packet is partial
- preserve contradictions until a higher-authority resolution exists
