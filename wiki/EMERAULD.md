---
type: wiki
title: EMERAULD
aliases:
- EMERAULD
- EMERAULD Second Brain — Project Context
- EMERAULD Persistence Stack
- EMERAULD — External Brain Vault
- wiki/EMERAULD
tags:
- emerauld
- vault
- context
- memory
- project
- wiki
- emerauld-md
- brain
- second
- rerouting
- periodic
- color-green
status: active
created: '2026-05-06'
updated: '2026-06-26'
vault_area: wiki
canonical_path: wiki/EMERAULD.md
backlink_count: 29
backlinks:
- '[[wiki/Architecture - EMERAULD Scripts - Intake Pipeline]]'
- '[[wiki/Architecture - EMERAULD Scripts - Key Decisions]]'
- '[[wiki/Architecture - EMERAULD Scripts - Knowledge Layers]]'
- '[[wiki/Architecture - EMERAULD Scripts - Overview]]'
- '[[Areas/PHAROS/Control Protocols MOC]]'
- '[[wiki/Fisher King Hub — Project Recovery Map]]'
- '[[wiki/Healing the Fisher King — Percephal Diagnostic Protocol]]'
- '[[Areas/PHAROS/Kickstart App Prompt — Template and Synthesis Framework]]'
- '[[Areas/Writing/Legitimacy Machines MOC]]'
- '[[wiki/Obsidian Second Brain Integration — EMERAULD Setup (2026-06-21)]]'
- '[[wiki/Rest and Consolidation Guide — Martin]]'
- '[[wiki/Stacklight-owner-explainer]]'
- '[[wiki/Trismégiste Eval v2 — Argus Closure and Redesign (2026-05-05)]]'
- '[[wiki/VAULT ADDITIONS TRACKER]]'
- '[[wiki/Vault Health — 2026-06-28]]'
- '[[wiki/Weekly Review — 2026-06-26]]'
- '[[_vault/VAULT ADDITIONS TRACKER]]'
- '[[artifacts/emerge-pattern-report-2026-06-29]]'
- '[[wiki/lightrag_config — EMERAULD Shared Config Module]]'
- '[[wiki/memory]]'
- '[[memory/daily/2026-06-22]]'
- '[[memory/daily/2026-06-23]]'
- '[[memory/daily/2026-06-24]]'
- '[[memory/daily/2026-06-25]]'
- '[[memory/daily/2026-06-26]]'
- '[[memory/daily/2026-06-27]]'
- '[[memory/daily/2026-06-28]]'
- '[[wiki/session-state]]'
- '[[wiki/trismegiste-state]]'
---

# EMERAULD

## Summary

EMERAULD is the local personal-knowledge and operator-continuity vault that holds the project context, persistence surfaces, and second-brain scaffolding for Martin's work. Its retrieval and graph layers are documented in [[EMERAULD Graph Architecture — Link Density and Vector Layer (2026-05-25)]], and its periodic structural-health reports — most recently [[Vault Health — 2026-07-05]] (prior: [[Vault Health — 2026-06-28]]) — track dangling links, orphans, and frontmatter debt. The June 2026 routing audit ([[WIKI-ROUTING-REPORT]]) identified 564 notes and produced destination recommendations for rerouting ~350 notes into the PARA area structure.

## Related

- [[Legitimacy Machines MOC]]
- [[Control Protocols MOC]]
- [[Home]]
- [[Obsidian Second Brain Product]]
- [[Obsidian Second Brain Integration — EMERAULD Setup (2026-06-21)]]
- [[claude-mem — Persistent Memory Compression for Claude Code]]
- [[PHAROS AI Lineage — Source of Truth]]
