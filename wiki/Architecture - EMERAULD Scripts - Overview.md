...
- **Every scheduled agent is a detector; none is a consumer.** Each script in `scripts/scheduled/` terminates in a write (daily note, stale artifact, session-state line, audit note, `FAILURES.md` entry). None opens a file a previous run produced and branches on its contents. The one exception is the deterministic remediation in `nightly.sh` for register thresholds and governance gates, which is why those two surfaces never drift and no other surface holds. Consequences on disk: one overdue pair flagged 21 mornings running with zero movement, `FAILURES.md` unread for two days, and a health audit that found 316 actionable broken links with no repair pass behind it. Full record, mechanism and candidate controls: [[EMERAULD Automation — Detection Without Consumption]].
