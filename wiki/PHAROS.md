---
type: wiki
title: PHAROS
aliases:
- PHAROS
- wiki/PHAROS
tags:
- pharos
- governance
- product
- method
- wiki
- pharos-md
- publication
- compassai
- track
- scholarly
- color-purple
status: active
created: '2026-05-06'
updated: '2026-06-26'
vault_area: wiki
canonical_path: wiki/PHAROS.md
backlink_count: 23
backlinks:
- '[[wiki/AI Paper Prompt — PHAROS Recursive Workflow Case Study]]'
- '[[wiki/APEX Papers — Research Archive Map]]'
- '[[wiki/AurorA — COMPASSai Input Module]]'
- '[[wiki/COMPASSai — Governance Engine]]'
- '[[Areas/PHAROS/Control Protocols MOC]]'
- '[[wiki/EMERAULD Graph Architecture — Link Density and Vector Layer (2026-05-25)]]'
- '[[Areas/PHAROS/Governance and PHAROS MOC]]'
- '[[Areas/PHAROS/HELIX Desktop Corpus — Protocol Evolution and Stress-Test Runs (2026-05-06)]]'
- '[[wiki/Hermes Dashboard — Professional Governance Tool]]'
- '[[wiki/Home]]'
- '[[wiki/Legitimacy Machines MOC]]'
- '[[wiki/MASTER PACK — D Drive Archive Map]]'
- '[[wiki/MASTERxMASTERxMASTER — Skill Corpus Map]]'
- '[[wiki/Manuscript Pipeline MOC]]'
- '[[wiki/Narrative and Method Integration — Creative Articulations of Governance]]'
- '[[wiki/Obsidian Second Brain Integration — EMERAULD Setup (2026-06-21)]]'
- '[[Areas/PHAROS/PHAROS AI Lineage — Source of Truth]]'
- '[[wiki/PHAROS Final Voice Operator — GPT Creator]]'
- '[[wiki/Research Hub]]'
- '[[_vault/VAULT ADDITIONS TRACKER]]'
- '[[maps/PHAROS Method Map]]'
- '[[memory/agents/Vibe]]'
- '[[session-state]]'
---

# PHAROS

## Summary

PHAROS is the recursive governance, commercial, and scholarly system that sits at the center of the EMERAULD vault's AI governance work.

## Cluster Frame

The active PHAROS graph has three useful clusters: method and evidence, product execution, and publication authority. The obsolete cluster boundary is "PHAROS as one giant governance MOC"; the stronger frame is a bridge between [[PHAROS Method — Technical Reference]], product evidence from [[COMPASSai — Governance Engine]], [[AurorA — COMPASSai Input Module]], and [[HELIX Desktop Corpus — Protocol Evolution and Stress-Test Runs (2026-05-06)]], and publication claims in [[Recursive Deterministic AI Governance — Method and Paper]] and [[PHAROS Scholarly Publication Track]].

## Inbound routing

Prefer this note as the PHAROS stack hub. Method detail: [[PHAROS Method — Technical Reference]]. Publication claims: [[PHAROS Scholarly Publication Track]] via the evidence cluster in [[Research Hub]]. Product evidence nodes should link here and to the publication track, not bypass through [[Governance and PHAROS MOC]] alone.

## Related

- [[Legitimacy Machines MOC]]
- [[Manuscript Pipeline MOC]]
- [[Control Protocols MOC]]
- [[Governance and PHAROS MOC]]
- [[PHAROS Method — Technical Reference]]
- [[PHAROS AI Lineage — Source of Truth]]
- [[PHAROS Commercial Strategy]]
- [[PHAROS Scholarly Publication Track]]
- [[Recursive Deterministic AI Governance — Method and Paper]]
- [[COMPASSai — Governance Engine]]
- [[AurorA — COMPASSai Input Module]]
- [[HELIX Desktop Corpus — Protocol Evolution and Stress-Test Runs (2026-05-06)]]
- [[Research Hub]]
