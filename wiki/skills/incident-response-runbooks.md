---
type: skill
title: incident-response-runbooks
aliases:
- skills/incident-response-runbooks
- wiki/skills/incident-response-runbooks
tags:
- skill
- wiki
- skills
- runbooks
- incident
- mortems
- chaos
- integrations
- color-teal
status: active
created: '2026-05-06'
updated: '2026-06-26'
vault_area: wiki
canonical_path: wiki/skills/incident-response-runbooks.md
backlink_count: 5
backlinks:
- '[[wiki/Skill Corpus — Complete Live Index (260 Active Skills)]]'
- '[[wiki/Skill Map — Canonical Routed Skills (2026-05-06)]]'
- '[[wiki/archive/ai-product]]'
- '[[wiki/skills/agent-management]]'
- '[[wiki/skills/hermes-integration-monitor]]'
skill_id: incident-response-runbooks
scope: routing
source_of_truth: ../governance/hephaistos/SKILL-MAP.md
---

# incident-response-runbooks

Canonical definition: see `governance/hephaistos/SKILL-MAP.md` (Routing).

**Purpose (vault-side):** design and execute incident runbooks, post-mortems, and chaos checks for agent systems and integrations.

## Related

- [[hermes-integration-monitor]]
- [[observability-governance]]
- [[Skill Map — Canonical Routed Skills (2026-05-06)]]

