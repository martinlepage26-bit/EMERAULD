---
type: wiki
title: Martin Lepage — Professional Profile
aliases:
- Martin Lepage — Professional Profile
tags:
- areas
- martin-lepage-professional-profile-md
- january
- creative
- certifications
- cgto
- stocker
- color-purple
status: active
created: '2026-06-21'
updated: '2026-06-26'
vault_area: Areas
canonical_path: Areas/PHAROS/Martin Lepage — Professional Profile.md
backlink_count: 116
backlinks:
- '[[.trash/Building Your First AI Agent with OpenAI_______]]'
- '[[wiki/A Lokian Family — Queer and Pagan Agency in Montreal (Pomegranate 2014)]]'
- '[[wiki/AI Governance Course — Ethics, Failure Modes, and Practice]]'
- '[[wiki/AI Personas — Agatha, DOTTIE, and MOBI]]'
- '[[Areas/Writing/Agatha All Along — Wicca, Digital Mediatization, and Proof Regimes]]'
- '[[Areas/Writing/Alchemy of the Wound — Novel]]'
- '[[wiki/Algorithmic Agentic AI and Governance — From Hegemonic Fluency to the Ethics of Interruption]]'
- '[[Areas/PHAROS/AurorA — COMPASSai Input Module]]'
- '[[wiki/Avatar Chapter — A Queer Reading of Embodied Power (English)]]'
- '[[wiki/Avatar — Lecture queer du pouvoir incarné]]'
- '[[wiki/Bonded Intelligence Under Constraint — The LOTUS Processor Framework]]'
- '[[Areas/Writing/Breath of the Astral Year — Astrology Monograph]]'
- '[[wiki/CDPDJ Complaint — Lepage v Calian and Novartis]]'
- '[[wiki/CLI-Anything — Agent Harness for Tool Integration]]'
- '[[wiki/CODEX Writing Projects Manifest]]'
- '[[Areas/PHAROS/COMPASSai — Governance Engine]]'
- '[[wiki/CORPUS ou le génie de l''insistance — Novel]]'
- '[[Areas/Writing/Complete Paper List — Martin Lepage Corpus]]'
- '[[Areas/PHAROS/ComplyScan — Bilingual Law 25 - AI Act Compliance SaaS]]'
- '[[wiki/Custom GPT Products — PHAROS AI GPT Roster]]'
- '[[wiki/Dr. Sort Codebase Version Genealogy — Three Iterations of Failed Automation]]'
- '[[wiki/Embedding Before Rupture — Relational AI and Institutional Power]]'
- '[[Areas/PHAROS/Emotional Alliance vs. Evidentiary Discipline in AI]]'
- '[[Areas/Writing/Ethics Poetics and Praxis — Braided Discipline]]'
- '[[wiki/Every Hair a Battle Scar — Buffy''s Hair as Narrative Engine]]'
- '[[wiki/Fisher King Hub — Project Recovery Map]]'
- '[[wiki/Global Publication Search — PHAROS Method and Variants]]'
- '[[wiki/Governance Stress-Test Protocols — Index]]'
- '[[wiki/Governance Typology — Recursive AI Governance Taxonomy]]'
- '[[Areas/PHAROS/Governance and PHAROS MOC]]'
- '[[wiki/HELIX Session — Vaisseau de Thésée and the Tressed Lie (Live Run 2026-04-26)]]'
- '[[Areas/PHAROS/HELIX — Value Proposition and Buyer Profile]]'
- '[[Areas/Writing/HENRY — Research Paper Writing System]]'
- '[[wiki/Halberstam — The Queer Art of Failure (2011)]]'
- '[[wiki/Healing the Fisher King — Percephal Diagnostic Protocol]]'
- '[[wiki/Healthcare Governance Packet — Recursive Governance for Providers]]'
- '[[wiki/Hermes Dashboard — Professional Governance Tool]]'
- '[[Areas/PHAROS/Historical Academic Portfolio — Pre-PHAROS Scholarly Work]]'
- '[[wiki/Home]]'
- '[[wiki/How French Canadians became White Folks — Scott 2016 (Race in Quebec)]]'
- '[[wiki/Intelligence Definitions and AI Bias — 1956 Lecture]]'
- '[[wiki/LOTUS Model — Agency and Social Positioning]]'
- '[[wiki/La fréquence brisée du verbe — Roman]]'
- '[[wiki/Le sang qui porte ton nom — Novel]]'
- '[[wiki/Legitimacy Machines Altars of Control — Full Book Architecture]]'
- '[[Areas/Writing/Legitimacy Machines — Book Skeleton and Architecture]]'
- '[[Areas/Writing/Loop Papers and Recursive Governance]]'
- '[[Resources/MASTER REFERENCE SAFE — Canonical Bibliography System]]'
- '[[wiki/Magic After Legitimacy — Charmed and the Governance of Female Power]]'
- '[[wiki/Martin Lepage Professional Identity]]'
- '[[wiki/Martin Lepage Publications — Annotated Bibliography and Verification Leads]]'
- '[[wiki/Martin Lepage — AI Governance Consulting Profile Assessment]]'
- '[[wiki/Martin Lepage — Adjacent Skill Ring]]'
- '[[wiki/Martin Lepage — Authored Skills]]'
- '[[wiki/Martin Lepage — Skills by Life Domain]]'
- '[[wiki/Martin Walks the Witches'' Road — Corpus as Charge-Persistence]]'
- '[[wiki/Masculinités et ritualités — La magie chez les Radical Faeries]]'
- '[[Resources/Master Bibliography — Références bibliographiques 2025]]'
- '[[wiki/Master Project Tracker — 2026]]'
- '[[wiki/Master Reference Bridge Atlas — 2026-05-06]]'
- '[[wiki/Mort et Naissance et L''Ombre et le Double — MA Thesis on Yvon Rivard (Université Laval, 2010)]]'
- '[[wiki/Mythocritique to PHAROS — The 2010 Master''s Thesis as Methodological Keystone]]'
- '[[wiki/Narrative, Identity and Academic Storytelling — Hyland 2018 (ILCEA)]]'
- '[[wiki/Neo-Shamanism and Gender — Montreal Pagan Community]]'
- '[[wiki/OpenAI Governance Framework — Comparison with PHAROS]]'
- '[[wiki/Operator-Check Skill — Burnout Cascade Interrupt]]'
- '[[wiki/PHAROS AI and Ethics Submission — Architecture Paper]]'
- '[[Areas/PHAROS/PHAROS Commercial Strategy]]'
- '[[wiki/PHAROS Legal Classification — CAE Code Strategy]]'
- '[[Areas/PHAROS/PHAROS Method — Technical Reference]]'
- '[[Areas/PHAROS/PHAROS Scholarly Publication Track]]'
- '[[wiki/PHAROS Workspace Inventory 2026-04-18]]'
- '[[Areas/PHAROS/PHAROS-AI Webservice — pharos-ai.ca]]'
- '[[wiki/Paganisme(s) queer — Lepage UQAM (REL2614, genre et Wicca)]]'
- '[[Areas/Personal/Personal and Projects MOC]]'
- '[[wiki/Pourquoi rêver encore — Lecture allégorique et métaphysique d''Yvon Rivard]]'
- '[[wiki/Privacy as Contextual Integrity — Nissenbaum 2004 (Public Surveillance)]]'
- '[[wiki/Prologue I Saw Willow Rise — Legitimacy Machines Opening]]'
- '[[wiki/Qualitative Handbook — Expanded Reference List]]'
- '[[wiki/Queer Neo-Pagan Spiritualities and the Question of Authenticity in Wicca]]'
- '[[wiki/Queering Neo-Pagan Magic — 2006 Paper]]'
- '[[wiki/Queering Neo-Pagan Magic — 2025 Revised Version]]'
- '[[wiki/Queering Neo-Pagan Magic — 2026 Paper]]'
- '[[wiki/Queering Neo-Pagan Magic — Identity Power and the Uses of Magic]]'
- '[[wiki/Queering Neo-Pagan Magic — MRW Submission (2026)]]'
- '[[wiki/Queerness and Transgender Identity — Lepage 2017 (Pagan Montreal, Wicca vs Reclaiming)]]'
- '[[wiki/Refusing Fixity — Aesthetic Governance and Fugitive Authorship]]'
- '[[Areas/Writing/Research and Papers MOC]]'
- '[[wiki/Rest and Consolidation Guide — Martin]]'
- '[[wiki/Rhétorique antique, mythos et IA — Gouvernance et sciences sociales]]'
- '[[Areas/Writing/Ritual Machine Lab — Legitimacy Machines Introduction]]'
- '[[wiki/SAKURA Project — Legitimacy, Mediation, and Ensoulment by Proxy]]'
- '[[wiki/Sealed Card Protocol — Legitimacy, Glitch, and Charging]]'
- '[[wiki/September 2024 Research Retrospective]]'
- '[[wiki/Skill Domain — Design and UX]]'
- '[[wiki/Skill Domain — Research and Scholarly Work]]'
- '[[wiki/Social Compass Reviewer Response — Wicca and Agatha All Along]]'
- '[[wiki/Social Stratification, Intersectionality, and the Why Be King Manuscript]]'
- '[[wiki/Stuttering through the Institution — Academic Containment and Queer Knowing]]'
- '[[wiki/The Broken Frequency of the Word — Novel]]'
- '[[wiki/The Circle Remains Open — Toward a Queer Sociology of the Sacred]]'
- '[[wiki/The Familiars of Hollywood — Queerness, Whiteness, and Representation]]'
- '[[wiki/The Inner Goddess — Gender Normativity and Gay Identity in Wiccan Ritualities]]'
- '[[wiki/The Returning Light — Monograph]]'
- '[[Areas/Writing/The Scythe Already in Motion — Buffy, Queer Ritual, and the Politics of Glitch]]'
- '[[wiki/The Weather Beneath the Walls — Novel]]'
- '[[wiki/Transmutation and the Body — Queer Ethics in Fullmetal Alchemist]]'
- '[[wiki/Trismégiste — Personal AI Assistant]]'
- '[[wiki/Vault Delta Interconnectivity Atlas — 2026-05-06]]'
- '[[wiki/Virtually Magical Literally — Queer Agency and the Digital Witch]]'
- '[[wiki/Wheels of Will — Ritual Bottle Art and Research-Creation Practice]]'
- '[[wiki/Who''s the Boob Who''s the Trap — AI Governance Through the Booby-Trap Device]]'
- '[[Areas/Writing/Why Be King Im Already a Queen — Book Project]]'
- '[[Areas/Writing/Writing and Novels MOC]]'
- '[[_vault/VAULT ADDITIONS TRACKER]]'
- '[[hephaistos/agents/Trismegiste Personal AI Assistant]]'
---

# Martin Lepage — Professional Profile

## Summary
AI Systems & Risk Mapping Consultant, PhD (UQAM 2017). President & Chief Governance and Trust Officer (CGTO) at PHAROS / InfraFabric. Clinical trial coordinator (Novartis/Calian, Oct 2025–Jan 2026). Multiple AI governance certifications (January 2026). Bilingual EN/FR, Québec/Canada focus. Co-inventor of the PHAROS method (with Danny Stocker). Author of academic papers, creative works, and governance frameworks, including the corpus carried by [[HEXA Press — Publishing and Practice Initiative]]. Related to [[PHAROS Invention Disclosure]], [[Founder Charter — Lepage and Stocker]], and [[Recursive Deterministic AI Governance — Method and Paper]].

This note is the public/professional surface in the Martin tier split. For the scholarly publication surface, see [[Martin Lepage Publications — Annotated Bibliography and Verification Leads]]; for the creative/compositional surface, see [[Writing and Novels MOC]].

## Graph Position

- [[Martin Lepage Professional Identity]] — top-level identity hub for the Martin graph split.
- [[Personal and Projects MOC]] — public/professional branch entry point.
- [[Research and Papers MOC]] — scholarly branch entry point.
- [[Writing and Novels MOC]] — creative/compositional branch entry point.
- [[Home]] — global vault entry surface that now surfaces the Martin split explicitly.

## Context
Source: `LEPAGE.AI.CV.2026.docx`. This note is the professional profile extracted from the CV for use as context across the EMERAULD knowledge system. ML's work spans AI governance, academic scholarship, creative writing, and applied consulting — the CV represents the full scope.

---

## Current Role and Focus

**Title**: AI Systems & Risk Mapping Consultant; President & CGTO, PHAROS Inc. / InfraFabric Research

**Primary domain**: AI governance — specifically the gap between formal AI accountability frameworks and actual deployment accountability

**Geographic focus**: Québec / Canada, with comparative international work

**Language**: Bilingual EN/FR (academic, professional, and creative work in both)

**Core competency as stated**: designing governance systems that produce deterministic, auditable accountability decisions from AI system outputs; the PHAROS method is the primary deliverable

---

## Education

**M.A., Université Laval, 2010**
Études littéraires, Faculté des lettres, Département des littératures. Thesis: *Mort et Naissance de Christophe Ulric et L'Ombre et le Double d'Yvon Rivard : lecture mythocritique d'une quête spirituelle*. See [[Mort et Naissance et L'Ombre et le Double — MA Thesis on Yvon Rivard (Université Laval, 2010)]] and the architectural keystone [[Mythocritique to PHAROS — The 2010 Master's Thesis as Methodological Keystone]] — the MA establishes the mythocritical-Hermetic methodological substrate that grounds LOTUS, PHAROS, the agent stack, and the EMERAULD vault.

**PhD, UQAM, 2017**
Université du Québec à Montréal. Dissertation in the area of ritual studies / media / ethnography (the foundation for the Social Compass paper on witchcraft — see [[Social Compass Paper — Ritual Authority and Régime de Preuve]]). The PhD is in the humanities/social sciences, not computer science — the AI governance turn is applied social science, not technical AI research. The PhD's sociocritique is the *extension* of the MA's mythocritique substrate, not its origin.

**AI Governance Certifications (January 2026)**
Multiple certifications acquired in January 2026. The clustering in January 2026 suggests a deliberate credential-building moment, likely coinciding with the launch of the PHAROS consulting practice and the InfraFabric founding (PHAROS Inc. registered April 2026 — see [[PHAROS Company Registration and Security Incidents]]).

---

## Selected Employment History

### AI Systems & Risk Mapping Consultant (Current)
Independent consulting. Clients in regulated sectors (healthcare implied by the Healthcare Governance Packet — see [[Healthcare Governance Packet — Recursive Governance for Providers]]). Core offering: the PHAROS method operationalized as a procurement-ready 21-day sprint (see [[PHAROS Procurement-Unblock Sprint]]).

### Clinical Trial Coordinator — Novartis/Calian (Oct 2025–Jan 2026)
Four-month contract in clinical research coordination. This is the direct precursor to the CDPDJ complaint (human rights complaint against Calian/Novartis — a separate file in raw sources, not yet processed). The clinical trial experience informs the healthcare governance work: ML has direct experience of how governance frameworks function (and fail) inside regulated research environments.

### Earlier Academic Career (Pre-2025)
The CV documents academic positions and publications across the 2010s–2020s. The Social Compass paper draws on 2013-2016 fieldwork; this suggests an active academic research program during this period. The creative works (novels, monographs) also emerge from this period.

---

## Key Publications (From CV)

**Academic**
- Social Compass paper (witchcraft / régime de preuve) — [[Social Compass Paper — Ritual Authority and Régime de Preuve]]
- RDAIG method paper — [[Recursive Deterministic AI Governance — Method and Paper]]
- Self-Polygraph Protocol / The Wheel and the Watcher papers — [[Self-Polygraph Protocol and Suprametacognition]], [[The Wheel and the Watcher]]
- Fluency/Interruption paper — [[Fluency, Interruption, and Institutional Accountability]]
- Governance by Denial working draft — [[Governance by Denial]]
- Recursive Continuity Without Memory manuscript — [[Recursive Continuity Without Memory — AI Identity Across Sessions]]
- 2017 charging objects paper (ritual/legitimacy) — referenced in [[Phase 1 Governance Formation Sequence]]
- Buffy studies papers (multiple) — referenced in [[Phase 1 Governance Formation Sequence]]

**Creative Works**
- *Alchemy of the Wound* (novel) — [[Alchemy of the Wound — Novel]]
- *The Returning Light* (philosophical-poetic monograph, 2025) — [[The Returning Light — Monograph]]
- *The Broken Frequency of the Word* (novel, 2025) — [[The Broken Frequency of the Word — Novel]]
- *THE BREATH OF THE ASTRAL YEAR* — [[Breath of the Astral Year — Astrology Monograph]]
- *CORPUS* (book/monograph, 2025) — [[CORPUS ou le génie de l'insistance — Novel]]
- *LE SANG QUI PORTE TON NOM* (French-language, 2025) — [[Le sang qui porte ton nom — Novel]]

**Governance / Technical**
- PHAROS Invention Disclosure (co-inventor with Stocker) — [[PHAROS Invention Disclosure]]
- Founder Charter — [[Founder Charter — Lepage and Stocker]]
- InfraFabric Architecture documents — [[InfraFabric Architecture]]

---

## The Profile as a Whole

The CV represents an unusual combination: PhD in humanities → academic scholar in ritual/media studies → AI governance consultant → co-founder of an AI infrastructure company → CGTO with IP ownership. The trajectory is coherent: the scholarly interest in how authority is produced, contested, and legitimated (régime de preuve, ritual authority, governance by denial) becomes the foundation for a practical method for governing AI authority claims.

The clinical trial coordinator role is the anomalous element — a four-month contract that interrupts the consulting trajectory. The CDPDJ complaint against Calian/Novartis suggests the role ended under adverse conditions, which may itself be relevant to the healthcare governance work (direct experience of governance failure in a regulated research environment).

---

## Insights

- The humanities PhD is not a liability for AI governance work — it is the credential for the specific contribution: governance methodology that handles contested authority, partial evidence, and narrative control, which are precisely the problems that technical AI safety research tends to undertheorize
- The January 2026 certification cluster suggests a deliberate repositioning moment: acquiring formal AI credentials to credentialize a practice that was already substantively developed
- The creative work volume (multiple novels + monographs in 2025 alone) suggests a practice where creative and scholarly production are parallel, not sequential — the novels are not sabbatical projects, they are concurrent with the governance work
- The CGTO role is a title that ML appears to have defined: it does not exist in standard corporate governance frameworks. The definition — "the person who owns the conditions under which the engine may be trusted" — is specific to the PHAROS architecture

## Open Questions

- What is the status of the CDPDJ complaint against Calian/Novartis?
- Have the 2025 creative works been published? What is the distribution channel for HEX/ADECIMAL Press?
- Which AI governance certifications were acquired in January 2026, and from which bodies?
- How is the academic portfolio (peer-reviewed papers) progressing — what is the submission/acceptance status for each?

## Sources
- `raw sources/LEPAGE.AI.CV.2026.docx`
- Related: [[Martin Walks the Witches' Road — Corpus as Charge-Persistence]] — canonical reading of the entire corpus as one charge expressed through recomposing surfaces; the single sentence that names what the CV cannot
- Related: [[Master Reference Bridge Atlas — 2026-05-06]] — Martin surface split and bibliography bridge map
- Related: [[PHAROS — Origin and Doctrine]] — the lived foundation; why the PHAROS program exists in Martin's own words
- Related: [[HEXA Press — Publishing and Practice Initiative]]
- Related: [[PHAROS Invention Disclosure]]
- Related: [[Founder Charter — Lepage and Stocker]]
- Related: [[Recursive Deterministic AI Governance — Method and Paper]]
- Related: [[Social Compass Paper — Ritual Authority and Régime de Preuve]]
- Related: [[PHAROS Company Registration and Security Incidents]]
- Related: [[PHAROS Procurement-Unblock Sprint]]

## Related

- [[The Familiars of Hollywood — Queerness, Whiteness, and Representation]]
- [[Building Your First AI Agent with OpenAI_______]]
- [[HISTORY]]
- [[2015 - correspondence]]
- [[2016 - Material needed for Anthropologica]]
- [[2017 - report]]
- [[535248.PDF]]
- [[FONDS GÉRARD-DION]]
- [[MARTIN LEPAGE (3)]]
- [[Identity]]
